...
def main_workflow(problem_description: str, task_lean_code: str = "") -> LeanCode:
    ...
    while "Lean Error" in execution_result and refinement_count < max_refinement_attempts:
        ...
        error_message = execution_result.split("Lean Error:")[1].strip() if "Lean Error:" in execution_result else execution_result
        ...
        if len(code_parts) > 1:
            ...
            clarification_prompt = [
                {"role": "system", "content": "You are an expert in Lean 4. Separate this solution into code and proof parts."},
                {"role": "user", "content": f"Separate this Lean 4 solution into clear code and proof parts:\n{refinement_solution}"}
            ]
    ...
    if "Lean Error" in execution_result:
        ...
        final_verification_prompt = [
            {"role": "system", "content": 
             "You are an expert in Lean 4 theorem proving and programming. "
             "Your task is to fix persistent errors in a Lean 4 implementation and proof."
            },
            {"role": "user", "content": 
             f"I have a Lean 4 implementation and proof with errors that previous refinements couldn't fix. "
             ...
             f"Current proof:\n```lean\n{generated_proof}\n```\n\n"
             f"Error message:\n{execution_result}\n\n"
             f"Please completely rewrite both the implementation and proof to make them work correctly. "
             f"Focus on simplicity and correctness. Return ONLY the implementation code and the proof, "
             f"clearly labeled and separated."
            }
        ]
        ...
        if "executed successfully" in final_execution_result:
            ...
            print(f"Warning: Final solution may still contain errors: {final_execution_result}")
